...
/*
 *  센서의 레이저 스캔을 담당하는 헤더파일.
 *  1. sensor_msgs namespace
 *    - LaserScan 구조체 정의: 헤더, 각도, 시간간격, 범위 등
 *    - 메세지 전송 탬플릿: LaserScan구조체 파일을 포함한 메세지 전송함수
 *  2. ros/message_traits
 *    - 메세지 정보: 메세지 여부, 고정된 크기여부, 헤더 존재여부를 std:TrueType나 std:FalseType으로 나타냄
 *    - MD5 checksum 정보 반환
 *    - 메세지 데이터타입: 메세지의 데이터타입을 반환, 현재 반환값은 sensor_msgs/LaserScan임
 *    - 메세지 헤더 정의: 메세지의 헤더 정의를 반환
 *  3. ros/serialization
 *    - 메세지의 각 필드를 stream에 지정된 순서대로 직렬화(serialization)한다.
 *  4. ros/message_operations
 *    - 메세지 스트림을 받아 각 항목에 맞게 출력한다.
 */
