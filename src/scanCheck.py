...
# 클러스터링 클래스
class Cluster:
    def __init__(self, sIndex, length, value):
        self.critValue = value
        self.cordInfo = []
        self.length = length
        self.sIndex = sIndex
        self.eIndex = sIndex

    def update(self, index, value, mean, std):
        zScore = abs((value-mean)/std);
        self.eIndex = index
        if self.sIndex > self.eIndex:
            self.sIndex -= self.length

        return zScore <= self.critValue

    def merge(self, start, end):
        self.sIndex = start
        self.eIndex = end

    def visualize(self, cordInfo):
        self.cordInfo = cordInfo[self.sIndex:self.eIndex+1]
